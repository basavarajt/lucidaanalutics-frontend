...
import { useAuth, useClerk, useUser } from '../contexts/AuthContext';
import { authApi, scoringApi, modelsApi, setAuthFailureHandler, setAuthTokenProvider } from '../api/client';
...
const DASHBOARD_SESSION_KEY = 'lucida_dashboard_session_v1';
...
const loadDashboardSession = () => {
  ...
  try {
    return JSON.parse(sessionStorage.getItem(DASHBOARD_SESSION_KEY) || '{}');
  } catch {
    return {};
  }
};

const saveDashboardSession = (patch) => {
  if (typeof window === 'undefined') {
    return;
  }
  ...
  try {
    ...
    sessionStorage.setItem(DASHBOARD_SESSION_KEY, JSON.stringify({ ...current, ...patch }));
  } catch {
...
const getSessionTokenWithRetry = async (getTokenFn, attempts = TOKEN_RETRY_ATTEMPTS) => {
...
export default function Dashboard() {
  const [sessionState] = useState(() => loadDashboardSession());
  ...
  // Tab State
  const [activeTab, setActiveTab] = useState(sessionState.activeTab || 'train'); // 'train' | 'score' | 'feedback' | 'models'
  ...
  const [trainingJobStatus, setTrainingJobStatus] = useState(null);

  // Uploader State
  const [files, setFiles] = useState([]);
  const [modelName, setModelName] = useState(sessionState.modelName || 'Ensemble-01');
  const [targetCol, setTargetCol] = useState(sessionState.targetCol || '');
  const [isHovering, setIsHovering] = useState(false);
  const [actionLoading, setActionLoading] = useState(false);
  const [error, setError] = useState(null);
  const fileInputRef = useRef(null);
  const [trainingMode, setTrainingMode] = useState(sessionState.trainingMode || 'supervised'); // 'supervised' | 'unsupervised'
  const [autoSelectModel, setAutoSelectModel] = useState(sessionState.autoSelectModel ?? true);
  const [selectModelFromList, setSelectModelFromList] = useState(sessionState.selectModelFromList ?? false);
  ...
  const [outcomeColumn, setOutcomeColumn] = useState(sessionState.outcomeColumn || '');
  const [feedbackWeight, setFeedbackWeight] = useState(sessionState.feedbackWeight ?? 2);
  ...
  }, [activeTab, modelName, targetCol, trainingMode, autoSelectModel, selectModelFromList, outcomeColumn, feedbackWeight]);
  ...
  useEffect(() => {
    ...
    setAuthTokenProvider(async () => getSessionTokenWithRetry(requestSessionToken, 3));
    setAuthFailureHandler(handleAuthFailure);

    const hydrateProfile = async () => {
      try {
        const token = await getSessionTokenWithRetry(requestSessionToken);
        if (!token) {
        ...
        setUser({
          email: profile.email || clerkUser?.primaryEmailAddress?.emailAddress || 'unknown@lucida.local',
          company_name: profile.company_name || clerkUser?.fullName || 'Lucida Workspace',
          tenant_id: profile.tenant_id || 'tenant-pending',
        ...
        setError('Authenticated, but profile sync failed. Please verify backend auth configuration.');
      } finally {
        if (!cancelled) {
          setLoading(false);
        }
  ...
  }, [clerkUser, isLoaded, isSignedIn, navigate, requestSessionToken, signOut]);
  ...
  useEffect(() => {
    if ((activeTab === 'score' || activeTab === 'feedback') && modelsArchive.length === 0) {
      loadModels();
    }
  ...
  useEffect(() => {
    ...
    const hasModel = modelsArchive.some((model) => model.model_name === modelName);
    if (modelName && hasModel) {
      loadModelIntel(modelName);
    } else {
      setSelectedModelIntel(null);
    }
  }, [activeTab, modelName, modelsArchive]);
  ...
  const loadModels = async () => {
    ...
    try {
      ...
      const loadedModels = resp.data.models || [];
      setModelsArchive(loadedModels);

      if (loadedModels.length > 0) {
        setModelName((current) => {
          const available = new Set(loadedModels.map((model) => model.model_name));
          if (current && available.has(current)) {
            return current;
          }
          return loadedModels[0].model_name;
        });
      } else {
        setSelectedModelIntel(null);
      }
  ...
  const executePipeline = async () => {
    if (!ensureCsvFilesSelected(files)) {
      setError("Please stage at least one CSV payload.");
      return;
    }
    const enteredModelName = (modelName || '').trim();
    if (activeTab === 'train' && !enteredModelName) {
      setError("Enter a model name before training.");
      return;
    }

    let modelPool = modelsArchive;
    if ((activeTab === 'score' || activeTab === 'feedback') && modelPool.length === 0) {
      modelPool = await loadModels();
      if (!modelPool) {
        return;
      }
    }

    const availableModelNames = new Set(modelPool.map((model) => model.model_name));
    if ((activeTab === 'score' || activeTab === 'feedback') && availableModelNames.size === 0) {
      setError("No trained models found for this workspace. Train a model first.");
      return;
    }

    let resolvedModelName = enteredModelName;
    if ((activeTab === 'score' || activeTab === 'feedback') && (!resolvedModelName || !availableModelNames.has(resolvedModelName))) {
      resolvedModelName = modelPool[0].model_name;
      setModelName(resolvedModelName);
    }

    if (activeTab === 'score' && !autoSelectModel && !availableModelNames.has(resolvedModelName)) {
      setError("Selected model was not found. Choose a trained model from Registry.");
      return;
    }

    if (activeTab === 'feedback' && !availableModelNames.has(resolvedModelName)) {
      setError("Feedback requires a trained model from Registry.");
      return;
    }
    ...
    const estimatedSeconds = estimatePipelineSeconds(activeTab, files);
    setEstimatedTime(estimatedSeconds);

    try {
      if (activeTab === 'train') {
        const queuedResp = await scoringApi.trainAsync(enteredModelName, files, targetCol || null, trainingMode);
        const jobId = queuedResp?.data?.job_id;

        if (!queuedResp?.success || !jobId) {
          throw new Error(queuedResp?.error?.message || 'Unable to queue training job.');
        }

        await pollTrainingCompletion({
          jobId,
          estimatedSeconds,
          getTrainingStatus: scoringApi.getTrainingStatus,
          onStatus: (statusData) => {
            setTrainingJobStatus(statusData);
          },
        });
        const rawTrainingResult = await fetchTrainingResult({
          jobId,
          getTrainingResult: scoringApi.getTrainingResult,
        });
        const normalizedTraining = normalizeTrainingResult(rawTrainingResult, files.length, trainingMode, enteredModelName);
        setTrainingData(normalizedTraining);
        loadModels();
      } else if (activeTab === 'score') {
        const resp = await scoringApi.score(resolvedModelName, files, autoSelectModel);
        setScoringData(resp.data);
      } else if (activeTab === 'feedback') {
        const resp = await scoringApi.feedback(resolvedModelName, files[0], outcomeColumn, autoRetrainEnabled, feedbackWeight);
        setFeedbackData(resp.data);
        loadModelIntel(resolvedModelName);
      }
      setFiles([]);
    } catch (err) {
      setError(extractDashboardErrorMessage(err, 'An unexpected anomaly occurred.'));
    } finally {
  ...
  const inspectMergePlan = async () => {
    ...
    try {
      ...
      setError(extractDashboardErrorMessage(err, 'Merge inspection failed.'));
    } finally {
  ...
  const handleRetrainFromFeedback = async () => {
    ...
    try {
      ...
      setError(extractDashboardErrorMessage(err, 'Feedback retrain failed.'));
    } finally {
      setActionLoading(false);
      setProgressType(null);
      setTrainingJobStatus(null);
  ...
  const handleSegmentRetrain = async (dimension, segment) => {
    ...
    try {
      ...
      setError(extractDashboardErrorMessage(err, 'Segment retrain failed.'));
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleDownloadCSV = () => {
    const result = downloadScoringCsv(scoringData, viewFilter);
    if (!result?.ok) {
          ...
          <section className="fade-in space-y-8">
            <div className="glass-card p-8 border border-line bg-surface/30">
              ...
              <ProgressBar 
                ...
                actualProgress={trainingJobStatus?.progress}
                stepLabel={trainingJobStatus?.current_step}
              />
